...
void serialEvent(Serial port) {   
  ...
  if (inData.charAt(0) == 'X') {
    ...
    appendArray( (sensorHist[0]), map(rawData[0], -80, 80, 0, height)); //store the data to history (for visualization)
    //calculating diff
  ...
  if (inData.charAt(0) == 'Y') {
    ...
    appendArray( (sensorHist[1]), map(rawData[1], -80, 80, 0, height)); //store the data to history (for visualization)
    //calculating diff
  ...
  if (inData.charAt(0) == 'Z') {
    ...
    appendArray( (sensorHist[2]), map(rawData[2], -80, 80, 0, height)); //store the data to history (for visualization)
    //calculating diff
